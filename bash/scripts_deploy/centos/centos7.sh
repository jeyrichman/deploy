...
if [ -e /usr/local/cpanel ]; then
	...
	yum -y install perl bzip2 smartmontools net-tools vim screen
...
#	wget http://data.king-support.com/distfiles/centos/centos-testing.repo -O /etc/yum.repos.d/centos-testing.repo
	wget -q http://data.king-support.com/distfiles/centos/epel-release.noarch.rpm -O /root/epel-release.noarch.rpm
...
echo "httpd and php"
	...
	wget -q http://data.king-support.com/distfiles/work7.tar.gz -O /usr/work/work.tar.gz
	...
	wget -q http://data.king-support.com/distfiles/Latest/httpd-2.4.12.tar.gz -O /usr/work/src/httpd-2.4.12.tar.gz
	wget -q http://data.king-support.com/distfiles/Latest/mod_geoip2-1.2.10.tar.gz -O /usr/work/src/mod_geoip2-1.2.10.tar.gz
	wget -q http://data.king-support.com/distfiles/Latest/APR/apr-1.5.1.tar.gz -O /usr/work/src/apr-1.5.1.tar.gz
	wget -q http://data.king-support.com/distfiles/Latest/APR/apr-util-1.5.4.tar.gz -O /usr/work/src/apr-util-1.5.4.tar.gz
	
	cd /usr/work/src
	tar zxfp /usr/work/src/php5.tar.gz
	tar zxfp /usr/work/src/httpd-2.4.12.tar.gz
	tar zxfp mod_geoip2-1.2.10.tar.gz && mv geoip-api-mod_geoip2-1.2.10 mod_geoip2-1.2.10
	tar zxfp apr-1.5.1.tar.gz
	tar zxfp apr-util-1.5.4.tar.gz
	cd apr-1.5.1/ && ./configure && make && make install && cd ../
	cd apr-util-1.5.4/ && ./configure --with-apr=/usr/local/apr && make && make install && cd ../
	ln -s /usr/work/src/php-5.* /usr/work/src/php5
	ln -s /usr/work/src/httpd-2.4.12 /usr/work/src/apache2
	/usr/work/src/do_apache.sh
	/usr/work/src/do_php5.sh
	wget -q http://data.king-support.com/distfiles/defaults/php5.ini -O /usr/local/lib/php.ini
	/usr/work/src/do_zend.sh
	mkdir /usr/local/apache2/conf/vhost/ /home/logs /etc/httpd 
	ln -s /usr/local/apache2/conf /etc/httpd/conf

	wget -q http://data.king-support.com/distfiles/defaults/httpd24.conf -O /usr/local/apache2/conf/httpd.conf
	wget -q http://data.king-support.com/distfiles/defaults/82port.conf.orig -O /usr/local/apache2/conf/vhost/82port.conf.orig
#	wget -q http://data.king-support.com/distfiles/centos/httpd.initd -O /usr/lib/systemd/system/httpd.service
...
#	systemctl enable httpd 
	...
	sed -i -e 's/Order Deny,Allow/Require all granted/g' /usr/local/apache2/conf/vhost/82port.conf
...
#fail2ban
	...
	echo -n "[sshd]" > /etc/fail2ban/jail.local
...
echo "nginx"
	...
	wget -q http://data.king-support.com/distfiles/centos/nginx.initd -O /usr/lib/systemd/system/nginx.service
